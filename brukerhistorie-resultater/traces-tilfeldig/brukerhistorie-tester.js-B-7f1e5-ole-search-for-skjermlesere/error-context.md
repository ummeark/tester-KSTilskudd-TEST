...
- Location: brukerhistorie-tester.js:793:3
...
> 796 |     await expect(searchRegion).toBeVisible({ timeout: SIDE_TIMEOUT });
  ...
  889 |   });
  890 | 
  891 |   test('AK-6 – Min side har gruppen "Avsluttede"', async ({ page }) => {
  892 |     await gåTilMinSide(page);
  893 |     await expect(
  894 |       page.locator('h2:has-text("Avsluttede"), h3:has-text("Avsluttede"), a:has-text("Avsluttede")').first()
  895 |     ).toBeAttached({ timeout: SIDE_TIMEOUT });
  896 |   });
```