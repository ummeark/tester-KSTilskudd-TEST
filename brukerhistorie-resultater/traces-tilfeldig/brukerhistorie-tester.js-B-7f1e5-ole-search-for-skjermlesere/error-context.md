...
- Name: brukerhistorie-tester.js >> BR.HIST-5: Som søker med hjelpemiddelteknologi vil jeg hoppe over navigasjonen >> søkeskjema er merket med role="search" for skjermlesere
- Location: brukerhistorie-tester.js:958:3
...
Locator: locator('[role="search"]').first()
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('[role="search"]').first()
...
> 964  |     await expect(searchRegion).toBeVisible({ timeout: SIDE_TIMEOUT });
  ...
  1056 |       expect(harSlettet, `"Slettet" skal ikke vises på ${sti}`).toBe(false);
  1057 |     }
  1058 |   });
  1059 | 
  1060 |   // AK-6 – tre grupper på Min side
  1061 |   test('AK-6 – Min side har gruppen "Utkast"', async ({ page }, testInfo) => {
  1062 |     testInfo.annotations.push({ type: 'steg', description: 'Navigere til /minside' });
  1063 |     await gåTilMinSide(page);
  1064 |     testInfo.annotations.push({ type: 'steg', description: 'Verifisere at gruppen "Utkast" finnes' });
```